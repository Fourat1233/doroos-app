...
import { usePaginatedFetch, useDebounce, useSearchQueryFetch } from '../shared/hooks'
...
const Title = ({ text }) => {
    ...
    return (
        <Text style={{ ...fonts.cairoBold, fontSize: 15, alignSelf: 'flex-start', paddingBottom: 3, paddingTop: 10 }}>
            {t(text)}
        </Text>
    )
}
...
const FieldWrapper = ({ children, style }) => {
    return (
        <View style={[{ borderWidth: 0.5, borderColor: '#dfe1e5', justifyContent: 'center', height: 55 }, style]}>
            {children}
        </View>
    )
}
...
export const SearchScreen = () => {
    const { load: loadFetchSearch, loading, countData: resultCount } = useSearchQueryFetch('teachers/search')
    ...
    const pressGenderHandler = (value) => dispatch({ type: SET_GENDER, payload: value })
