...
const HeaderRight = ({onClickDone}) => {
  return (
    <Item title="done" buttonStyle={styles.button} onPress={onClickDone} />
  );
};

const HeaderLeft = ({onClose}) => (
  <Item title="close" buttonStyle={styles.button} onPress={onClose} />
);
...
export const AppPicker = () => {
  ...
  const renderItem = ({item}) => (
    <FlatlistItem title={item.address} id={item.id} />
  );
      ...
      <TouchableHighlight
        ...
        <View style={styles.chooseButton}>
          <Text style={styles.chooseText}>
            {locations.length > 0
              ? `(${locations.length}) Areas choosed`
              : `Choose Areas`}
          </Text>
          <MaterialCommunityIcons
            ...
            style={styles.switchChoooseIcon}
          />
        </View>
      </TouchableHighlight>
    </View>
