...
import { StyleSheet, View, Modal, TouchableHighlight, TextInput, FlatList, Text } from 'react-native';
...
const HeaderRight = ({ onClickDone }) => (<ItemIcon title='done' buttonStyle={styles.button} onPress={onClickDone} />)
const HeaderLeft = ({ onClose }) => (<ItemIcon title='close' buttonStyle={styles.button} onPress={onClose} />)
const HeaderActions = ({ children, height }) => (<View style={[styles.headerButtons, { height }]}>{children}</View>)
        ...
        >
            <>
                ...
                <View style={[styles.checkBox, { backgroundColor: colors.base }]}>
                    {checked && <FontAwesome name='check' color={colors.white} size={14} />}
                </View>
            </>
        </TouchableHighlight>
    )
...
const Picker = ({ visible, uri, onVisibleChange, onCheckedItems }) => {
    ...
    const checkItemHandler = (item) => {
        if (exist(item.id)) { setCheckedItems(checkedItems.filter((checkedItem) => checkedItem.id !== item.id)) }
        else { setCheckedItems([...checkedItems, item]) }
    }

    const renderItem = useCallback(({ item }) => (<Item item={item} onCheckItem={checkItemHandler} checked={exist(item.id)} />))
